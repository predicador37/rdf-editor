...
La implementación del diseño del software se compara habitualmente con la construcción de un edificio basándose en sus planos, pero lo cierto es que el desarrollo de software es una disciplina eminentemente creativa que hace que dicha comparación no termine de ser realmente representativa. Por otra parte, en la filosofía del \textit{Test Driven Development} (TDD) \cite{beck02}, primero se codifica la prueba unitaria y después se implementa la lógica a probar. Este enfoque permite garantizar niveles razonables de calidad en el desarrollo de software. En el presente proyecto no se ha seguido estrictamente debido a la naturaleza del desarrollo, muy orientado a la interfaz gráfica; no obstante, sí se han incluido las pruebas unitarias consideradas como críticas para garantizar un correcto funcionamiento general de la aplicación. El presente capítulo expone detalles relevantes de implementación así como una relación de las pruebas unitarias y extremo a extremo codificadas para garantizar la calidad del producto.
...
Es importante describir dicha evolución a través de sus distintos estándares (\textit{ECMAScript}) para contextualizar el presente desarrollo:
...
Aunque los navegadores están haciendo grandes progresos con su compatibilidad total con ES6, aún no es recomendable utilizarlo en forma nativa. Para resolver este problema surgen los transpiladores o \textit{transpilers} como Babel\footnote{\url{https://babeljs.io/}} (utilizado en el presente proyecto), que traducen el lenguaje programado en un estándar más moderno como \textit{ES2015+} a \textit{``Vanilla ES5''}, de tal manera que el código pueda ser ejecutado en cualquier navegador.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{services}: contiene la configuración única de servicios para recuperar información de APIs \textit{Rest} a través de bibliotecas como \textit{axios}\footnote{Un cliente HTTP basado en promesas para Node.js. Ver \url{https://github.com/axios/axios}}
		\item \textbf{store}: contiene la lógica necesaria para mantener el estado único de la aplicación mediante \textit{Vuex} (ficheros de acciones, mutaciones, getters y el propio almacén.)
...
\begin{itemize}
	...
	\item Construcción:
		...
		\item Minificación de HTML con \textit{html-minifier}.
		\item Minificación de CSS con \textit{cssnano}.
...
Para desplegar la aplicación a modo de demostración se ha optado por utilizar \textit{Firebase}\footnote{\url{https://firebase.google.com/}}, una plataforma de desarrollo de \textit{Google}, a modo de PaaS\footnote{\textit{Platform as a Service}: entorno de desarrollo e implementación en la nube.}. Si bien \textit{Firebase} provee de muchos servicios (como base de datos, autenticación, almacenamiento, etc.), para este proyecto tan solo ha sido necesario utilizar el servicio de alojamiento. Para ello, basta con incluir un par de sencillos ficheros de configuración con el siguiente contenido:
...
Dado el abrumador número de ontologías disponibles\footnote{Ejemplo: lista de ontologías de Protégé en \url{https://protegewiki.stanford.edu/wiki/Protege_Ontology_Library}}, para la implementación de la funcionalidad de modelado en el presente proyecto se ha optado por incluir en memoria un subconjunto de ternas pertenecientes a los vocabularios más utilizados. Usando como referencia \cite{evanstaylor01}, las sugerencias del Director del proyecto y criterios relacionados con el alcance de la funcionalidad implementable para esta primera versión, se ha decidido dar soporte a las estructuras listadas en el cuadro \ref{tab:model-constructs}
	...
	\begin{tabular}{p{4cm}p{10cm}}
		...
		\textit{rdf:XMLLiteral} &  Clase de todos los valores literales de XML. \\ \midrule
...
Sin pretender entrar a fondo en los detalles de la implementación, que puede consultarse en el repositorio de código fuente del proyecto en: \url{https://github.com/predicador37/rdf-editor}, sí se ha considerado conveniente documentar determinadas decisiones:
	...
	\end{itemize}
\item En el módulo de actividades, la importación a \textit{Markdown} se realiza mediante la biblioteca \textit{vue-markdown}\footnote{https://github.com/miaolz123/vue-markdown}, que a su vez utiliza \textit{markdown-it}\footnote{https://github.com/markdown-it/markdown-it}. Esta última biblioteca implementa un procesador del formato \textit{Markdown} con buen rendimiento y conforme a la especificación \textit{CommonMark Spec} 0.28\footnote{\url{https://spec.commonmark.org/}}, si bien añade extensiones de sintaxis (como tablas y tachados de \textit{Github Flavored Markdown}\footnote{La sintaxis de Markdown propuesta por Github, que añade características como tablas, estilo de texto, etc. Ver \url{https://help.github.com/articles/basic-writing-and-formatting-syntax/\#styling-text}}) y otros aspectos.
