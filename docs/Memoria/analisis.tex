...
\begin{tabular}{p{1cm}p{2cm}p{10cm}}
	...
	5 & 4/08/18 & Revisión de los últimos avances con la integración de un endpoint SPARQL en el \textit{frontend }y planificación del resto de funcionalidade requeridas.  \\ 
	\bottomrule
\end{tabular}
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item El usuario introduce el URI del recurso que quiere dar de alta como clase/propiedad y pulsa en guardar.
...
\begin{enumerate}
	...
	\item Aparece un cuadro de diálogo donde el usuario debe introducir el nombre del recurso.
	\item El usuario introduce (o no) el URI del recurso que quiere dar de alta como clase/propiedad y pulsa fuera del cuadro de diálogo.
...
\end{enumerate}
	...
	\begin{enumerate}
		\item El usuario pulsa en el icono de menú asociado al recurso que quiere editar y selecciona la acción ``Editar''.
		\item Aparece un cuadro de diálogo con el URI del recurso actual que el usuario puede modificar.
		\item El usuario modifica el URI del recurso  y pulsa en guardar.
		\item Ocurre un error en el guardado del recurso que le es comunicado al usuario a través de una notificación en pantalla.
	\end{enumerate}
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item El usuario introduce el URI del recurso que quiere dar de alta como anotación y pulsa en guardar.
	...
	\begin{enumerate}
		\item El usuario pulsa en el botón flotante + asociado al panel de anotaciones.
		\item Aparece un cuadro de diálogo donde el usuario debe introducir el nombre de la anotación.
		\item El usuario introduce (o no) el URI del recurso que quiere dar de alta como anotación y pulsa fuera del cuadro de diálogo.
		\item El cuadro de diálogo se cierra y no hay ningún cambio en los recursos.
	\end{enumerate}
	\item Flujo alternativo 2
	\begin{enumerate}
...
\begin{enumerate}
	...
	\item Flujo alternativo 2
	\begin{enumerate}
		\item El usuario pulsa en el icono de menú asociado a la anotación del recurso que quiere eliminar y selecciona la acción ``Editar''.
		\item Aparece un cuadro de diálogo advirtiendo al usuario de que si confirma el cambio, no podrá volver a recuperar el recurso del grafo.
	\item El usuario elige cancelar.
	\item El recurso y su detalle siguen apareciendo y no son eliminados.
	\end{enumerate}
...
El caso de usa comienza cuando el usuario selecciona un recurso en la tarjeta que los lista y pulsa en el icono del menú para seleccionar ``Añadir subclase''.

\subsubsection{Flujo de eventos}
\begin{enumerate}
	\item Flujo básico
	\begin{enumerate}
		\item El usuario selecciona un recurso en la tarjeta que los lista y pulsa en el icono del menú para seleccionar ``Añadir subclase''.
		\item Aparece un cuadro de diálogo donde el usuario debe introducir el nombre del recurso derivado a crear.
		\item El usuario introduce el URI del recurso que quiere dar de alta como derivado y pulsa en guardar.
		\item El nuevo recurso se crea y aparece en el listado de anotaciones correspondiente.
	\end{enumerate}
	\item Flujo alternativo 1
	\begin{enumerate}
		\item  El usuario selecciona un recurso en la tarjeta que los lista y pulsa en el icono del menú para seleccionar ``Añadir subclase''.
		\item Aparece un cuadro de diálogo donde el usuario debe introducir el nombre del recurso derivado a crear.
		\item El usuario introduce (o no) el URI del recurso que quiere dar de alta como derivado y pulsa fuera del cuadro de diálogo.
		\item El cuadro de diálogo se cierra y no hay ningún cambio en los recursos.
	\end{enumerate}
	\item Flujo alternativo 2
	\begin{enumerate}
...
La actividad se visualizará con el formato \textit{Markdown} interpretado en HTML en la tarjeta asociada al contenido de la misma.
...
Es necesario que haya conexión a Internet para poder descargar los vocabularios de las URLs precargadas así como que los servidores remotos donde residen los vocabularios presenten una configuración correcta de las cabeceras CORS (\textit{Cross-Origin Resource Sharing})\footnote{\url{https://developer.mozilla.org/es/docs/Web/HTTP/Access_control_CORS}}
...
Este caso de uso permite a un usuario importar un grafo completo en los formatos \textit{N3, Turtle, TriG o N-Triples}. El grafo importado sustituirá al cargado por defecto en memoria por la aplicación.
...
Este caso de uso  permite a un usuario exportar a un archivo de texto en formato \textit{Turtle} o \textit{JSON-LD} el grafo de trabajo.
...
Este caso de uso  permite a un usuario lanzar una consulta SPARQL al grafo local cargado en memoria, bien desde un archivo con extensión \textit{``.rq''} o URL, bien desde un cuadro de texto.
...
\begin{enumerate}
	\item Flujo básico
	\begin{enumerate}
		\item El usuario carga la consulta a lanzar, bien escribiéndola en el cuadro de texto, bien cargándola desde un archivo \textit{``.rq''} o URL donde se encuentre.
...
Este caso de uso  permite a un usuario lanzar una consulta SPARQL a un grafo ubicado en un \textit{endpoint} remoto, bien desde un archivo con extensión \textit{``.rq''} o URL, bien desde un cuadro de texto.
...
El caso de usa comienza cuando el usuario activa la opción ``Usar \textit{endpoint} SPARQL remoto''.

\subsubsection{Flujo de eventos}
\begin{enumerate}
	\item Flujo básico
	\begin{enumerate}
		\item El usuario activa la opción ``Usar \textit{endpoint} SPARQL remoto''.
		\item El usuario introduce la URL del \textit{endpoint} remoto.
		\item El usuario carga la consulta a lanzar, bien escribiéndola en el cuadro de texto, bien cargándola desde un archivo \textit{``.rq''} o URL donde se encuentre.
		\item El usuario pulsa en el botón de ``Lanzar Consulta''.
