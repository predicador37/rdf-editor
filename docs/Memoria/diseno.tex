...
\begin{itemize}  
	\item \textbf{Almacén de ternas} (\textit{store}): no es más que una base de datos que permite el almacenamiento y extracción de ternas \gls{RDF}, así como consolidar información procedente de distintas fuentes de datos.
...
\subsection{Almacén \gls{RDF} (\textit{store})}

El almacén de ternas es un componente esencial para salvaguardar conjuntos de datos en ternas independientemente de su origen (un fichero serializado, el resultado de una consulta, etc.). Conceptualmente, el equivalente en una base de datos relacional sería una tabla con tres columnas (\textit{Sujeto - Predicado - Objeto}). Sin embargo, parece evidente que un almacén \gls{RDF} esté optimizado para el almacenamiento y recuperación de ternas \gls{RDF}.
...
\begin{itemize}  
	...
	\item \textbf{Estandarización:} el nivel de estandarización de \gls{RDF} y \gls{SPARQL} es mucho mayor que el de \gls{SQL}. Es prácticamente inmediato sustituir un almacén de tripletas por otro, mientras que en el caso de las bases de datos relacionales es necesario tener en cuenta los distintos dialectos e implementaciones de \gls{SQL} según el fabricante.
	\item \textbf{Expresividad:} es mucho más sencillo modelar datos complejos y con elevado número de relaciones entre ellos en \gls{RDF} que en \gls{SQL}. Ocurre al contrario si los datos son de naturaleza tabular, naturalmente.
\end{itemize}
...
\begin{itemize}  
	\item \textit{\textbf{RDF-Ext Dataset\footnote{\url{https://github.com/rdf-ext/rdf-store-dataset}}}} un simple conjunto de datos cargado en memoria y conforme con la especificación estándar. Este almacén presentaba limitaciones en cuanto a rendimiento y número de tripletas a almacenar, además de estar pensado para operaciones síncronas.
	\item \textit{\textbf{N3.js:\footnote{\url{https://github.com/rdfjs/N3.js/}}}} La bilioteca \textit{N3.js} ofrece, además de almacenamiento de tripletas en memoria en \textit{Javascript} nativo, facilidades de serialización y procesado con formatos estándar como \textit{Turtle, \gls{N3}, N-Triples y TriG}. Si bien su conformidad con la especificación de interfaces\textit{ RDF.js} no es completa, sí está presente en gran parte de sus módulos (\textit{DataFactory, StreamParser, StreamWriter y Store}).
\end{itemize}

Confirmadas las limitaciones de \textit{RDF-Ext Dataset} y consultado con el \textit{\gls{W3C} RDFJS Community Group}, se opta finalmente por utilizar \textbf{\textit{N3.js}} (lo que supuso adaptar ligeramente la implementación del proyecto al nuevo \textit{storage}).


\subsection{Procesador y Serializador \gls{RDF} (\textit{parser/serializer})}

En muchos casos, los módulos para importación y exportación de datos en \gls{RDF} son proporcionados por el propio almacén. Sin entrar a valorar su sintaxis dada su complejidad específica y por considerarse fuera del interés de este proyecto, sí se va a proceder a identificar y caracterizar los formatos de serialización más comunes.

\subsubsection{\gls{RDF}/\gls{XML}}

Se trata de una representación en \gls{XML}, comúnmente criticada por su dificultad de lectura por parte de personas. Entre sus otras críticas están las limitaciones impuestas por sus reglas de nomenclatura o los problemas encontrados para procesar este formato con herramientas populares.\cite{ducharme01}. Para poder codificar un grafo \gls{RDF} en \gls{XML}, tanto los nodos como los predicados han de ser representados en términos de \gls{XML} (elementos, atributos, contenido de elementos y valores de atributos). 

Conceptualmente, se construye a partir de un conjunto de descripciones más pequeñas, cada una de las cuales traza un camino a lo largo de un grafo \gls{RDF}. Estos caminos se describen en términos de los nodos (sujetos) y enlaces (predicados), que los conectan con otros nodos (objetos).

A continuación se muestra un ejemplo de serialización en \gls{RDF}/\gls{XML}\footnote{\url{https://www.w3.org/TR/rdf-syntax-grammar/\#example3}}:
...
\textit{\gls{N3}} fue un proyecto personal de Tim Berners-Lee\cite{ducharme01}. Es muy similar a \textit{N-Triples}, pero añadiendo características adicionales como atajos, un formato más claro, o la condensación de muchas de las repeticiones de este formato.

A pesar de todo, \textit{\gls{N3}} nunca se convirtió en un estándar y sus características adicionales frente a \textit{N-Triples} no tuvieron mucha aceptación.

\subsubsection{\textit{Turtle}}

El formato Turtle permite escribir un grafo \gls{RDF} en texto de una forma más compacta que \gls{RDF}/\gls{XML} y más sencilla de leer que \textit{N-Triples}. Su gramática es un subconjunto de la especificación del lenguaje de consulta \gls{SPARQL} 1.1, compartiendo ambas nombres de terminales y producciones en la medida de lo posible. En estos momentos, es el formato de serialización más popular entre las comunidades de desarrolladores\cite{yu01}.

A continuación se muestra un ejemplo de serialización en \textit{Turtle}\footnote{https://www.w3.org/TR/turtle/}:
...
El almacén seleccionado, \textbf{\textit{N3.js}, proporciona procesadores y serializadores} para los principales formatos citados: \textit{\gls{N3}, N-Triples, Turtle y TriG} (una extensión de este último).

No se ha considerado necesario diseñar ningún serializador o procesador personalizado, al estar ya incluidos los formatos más comunes utilizados por desarrolladores (nótese la ausencia deliberada de \gls{RDF}/\gls{XML}).


\subsection{Motor de consulta \gls{RDF} (\textit{query engine})}

El motor de consulta es un componente íntimamente ligado al almacén de ternas. El \gls{W3C} fijó un estándar para consultas: \gls{SPARQL} (presentado en \ref{sec:sparql}), cuya especificación está en su versión 1.1 desde 2013.
...
Los principales proveedores de almacenes de ternas ofrecen también motores de consulta integrados, como \textit{Apache Jena, Virtuoso, AllegroGraph...}
...
Tras el estudio de los trabajos previos desarrollados (\ref{subsec:consulta}) y la  situación actual (\ref{subsubsec:manejosparql}), se llegó a la conclusión de que la plataforma \textit{Comunica} se posicionaba como el producto abierto más prometedor para satisfacer las necesidades de consulta \gls{SPARQL}.
...
De forma resumida, un cliente que soporta este tipo de fragmentos puede resolver múltiples consultas \gls{SPARQL} de forma eficiente.
...
\begin{figure}[h]
	...
	\caption{Arquitectura de \textit{Comunica}. Fuente: Web de documentación de \textit{Comunica}.}
\end{figure}

Para el presente proyecto, han sido necesarios dos actores:

\begin{itemize}  
	\item \textbf{actor-init-sparql:} se trata de un cliente que permite resolver consultas sobre interfaces heterogéneos. En concreto, este módulo inicializa un motor \textit{Comunica} con actores que evalúan consultas \gls{SPARQL}.
...
Es decir, se utiliza un módulo para lanzar consultas a \textit{endpoints} \gls{SPARQL} remotos y otro para la fuente o almacén local. Estos módulos \textbf{aún están en desarrollo y no han alcanzado una versión estable} a fecha de redacción de la presente memoria, con lo que se van adquiriendo nuevas funcionalidades y corrigiendo defectos a lo largo del tiempo de desarrollo de este proyecto. Por ejemplo:

\begin{itemize}  
	\item En Julio de 2018 el motor de consulta no soportaba la palabra clave SERVICE, característica implementada en Agosto
	\item En Septiembre de 2018 el motor de consulta no soportaba agregados como COUNT, si bien su incorporación estaba prevista para un par de meses después.
\end{itemize}


\section{Arquitectura de la aplicación Vue}

Al margen de su naturaleza semántica, la herramienta desarrollada se implementa en \textit{Javascript ES6+} y \textit{Vue.js}. Es necesario, por tanto, proponer un diseño de alto nivel de sus componentes.

\subsection{Almacenamiento de estado}

Para almacenar el estado de la aplicación se ha optado por usar \textit{Vuex}\footnote{\url{https://vuex.vuejs.org/}}. 

\textit{Vuex} es una biblioteca que implementa un patrón de gestión de estado (ver figura \ref{fig:vuex}\cite{vuex01}) basado en \textit{Redux}\footnote{\textit{Redux} es un contenedor predecible del estado de aplicaciones \textit{Javascript}. Ver \url{https://es.redux.js.org/}}, \textit{Flux}\footnote{Una arquitectura de aplicación para \textit{React} que utiliza un flujo de datos unidireccional. Ver \url{https://github.com/facebook/flux}} y la arquitectura \textit{Elm}\footnote{Un patrón sencillo de arquitectura de aplicaciones web. Ver \url{https://guide.elm-lang.org/architecture/}}. Se utiliza como un almacén centralizado para todos los componentes de la aplicación, y permite asegurar:
...
\begin{figure}[h]
	...
	\caption{\textit{Vuex}: gestión de estado centralizado.}
	\label{fig:vuex}
\end{figure}

Sin entrar en detalles, el funcionamiento de \textit{Vuex} se ciñe a las siguientes reglas:
...
\begin{itemize}  
	...
	\item El estado sólo puede ser modificado mediante de \textbf{mutaciones síncronas} (\textit{mutations}).
	\item Las mutaciones pueden ser ejecutadas de manera asíncrona a través de \textbf{acciones} (\textit{actions}).
\end{itemize}
...
\begin{table}
	...
	\begin{tabular}{p{3cm}p{10cm}}
		...
		Bloques de construcción \gls{RDF} &  Contiene los bloques más comunes utilizados para modelar un dominio de información en \gls{RDF} (ej: clases, propiedades, etc.) \\ \midrule
		...
		Prefijos & Contiene los prefijos más comunes utilizados para precargar en las consultas \gls{SPARQL}. \\ \midrule
		Vocabularios &  Contiene las definiciones y \gls{URL}s de los vocabularios más comunes para posibilitar su descarga al grafo en el módulo correspondiente. \\
		\bottomrule
	\end{tabular}
...
\end{table}
	...
	\begin{tabular}{p{2cm}p{3cm}p{11cm}}
		...
		ACC-8 & Añadir \textit{\gls{N3}} &  Añade al grafo existente otro en formato \textit{\gls{N3}, Turtle, TriG o N-Triples}. \\ \midrule
...
En el cuadro \ref{tab:modulos} se plantea el desglose de la aplicación en módulos independientes que dan respuesta a las necesidades expuestas y a los casos de uso concretos.
...
Para poder dar cumplimiento a los requisitos de la aplicación, se definen en el cuadro \ref{tab:componentes} los componentes necesarios con sus ámbitos de responsabilidad.
	...
	\begin{table}
		...
		\begin{tabular}{p{2cm}p{5cm}p{8cm}}
			...
			COMP-5 & Contenedor de consultas \gls{SPARQL} &  Contiene los componentes de ejecución y visualización de resultados de consultas \gls{SPARQL}. \\ \midrule
			...
			COMP-16 & Acerca de & Componente con información relevante sobre la aplicación. \\ \midrule
...
En la figura \ref{fig:componentes} se muestra un diagrama \gls{UML} 2.0\cite{fowler01} con la jerarquía de componentes de la aplicación, que muestra convenientemente las relaciones entre los mismos distinguiéndose tres niveles:
...
\begin{itemize}
	...
	\item Componentes finales reutilizables. Son los últimos niveles de la jerarquía y realizan tareas muy concretas e independientes.
\end{itemize}
