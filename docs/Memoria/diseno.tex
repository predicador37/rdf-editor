...
\begin{itemize}  
	...
	\item \textit{\textbf{N3.js:\footnote{\url{https://github.com/rdfjs/N3.js/}}}} La bilioteca \textit{N3} ofrece, además de almacenamiento de tripletas en memoria en \textit{Javascript} nativo, facilidades de serialización y procesado con formatos estándar como \textit{Turtle, N3, N-Triples y TriG}. Si bien su conformidad con la especificación de interfaces\textit{ RDF.js} no es completa, sí está presente en gran parte de sus módulos (\textit{DataFactory, StreamParser, StreamWriter y Store}).
\end{itemize}

Confirmadas las limitaciones de \textit{RDF-Ext Dataset} y consultado con el \textit{W3C RDFJS Community Group}, se opta finalmente por utilizar \textbf{\textit{N3.js}} (lo que supuso adaptar ligeramente la implementación del proyecto al nuevo \textit{storage}).
...
Se trata de una representación en XML, comúnmente criticada por su dificultad de lectura por parte de personas. Entre sus otras críticas están las limitaciones impuestas por sus reglas de nomenclatura o los problemas encontrados para procesar este formato con herramientas populares.\cite{ducharme01}. Para poder codificar un grafo RDF en XML, tanto los nodos como los predicados han de ser representados en términos de XML (elementos, atributos, contenido de elementos y valores de atributos). 
...
\subsubsection{\textit{N3}}

\textit{N3}, abreviatura de \textit{Notation 3}, fue un proyecto personal de Tim Berners-Lee\cite{ducharme01}. Es muy similar a \textit{N-Triples}, pero añadiendo características adicionales como atajos, un formato más claro, o la condensación de muchas de las repeticiones de este formato.

A pesar de todo, \textit{N3} nunca se convirtió en un estándar y sus características adicionales frente a \textit{N-Triples} no tuvieron mucha aceptación.
...
El almacén seleccionado, \textbf{\textit{N3.js}, proporciona procesadores y serializadores} para los principales formatos citados: \textit{N3, N-Triples, Turtle y TriG} (una extensión de este último).
...
En el caso del presente proyecto, integrar un motor de consultas no fue sencillo. El componente \textit{N3.js} no ofrece dichas funcionalidades, con lo que fue necesario estudiar la viabilidad de incorporar un motor local totalmente desarrollado en \textit{Javascript}.

Tras el estudio de los trabajos previos desarrollados (\ref{subsec:consulta}) y la  situación actual (\ref{subsubsec:manejosparql}), se llegó a la conclusión de que la plataforma \textit{Comunica} se posicionaba como el producto abierto más prometedor para satisfacer las necesidades de consulta SPARQL.
...
\begin{itemize}  
	\item \textbf{actor-init-sparql:} se trata de un cliente que permite resolver consultas sobre interfaces heterogéneos. En concreto, este módulo inicializa un motor \textit{Comunica} con actores que evalúan consultas SPARQL.
	\item \textbf{actor-init-sparql-rdfjs:} permite lanzar consultas SPARQL a fuentes que implementan la interfaz \textit{Source}\footnote{\url{http://rdf.js.org/\#source-interface}}. Una de esas fuentes es \textit{N3.js}, el \textit{store} utilizado para almacenar ternas en el proyecto.
\end{itemize}
...
\begin{itemize}  
	...
	\item El estado sólo puede ser modificado mediante de \textbf{mutaciones síncronas}.
...
\end{table}
	...
	\begin{tabular}{p{2cm}p{3cm}p{11cm}}
		...
		ACC-7 & Importar \textit{N3} & Importa un grafo en formato \textit{N3, Turtle, TriG o N-Triples}. \\ \midrule
		ACC-8 & Añadir \textit{N3} &  Añade al grafo existente otro en formato \textit{N3, Turtle, TriG o N-Triples}. \\ \midrule
		ACC-9 & Eliminar \textit{N3} & Elimina del grafo local las ternas existentes en otro grafo en formato \textit{N3, Turtle, TriG o N-Triples} \\ \midrule
		ACC-10 & Cargar actividad & Carga el contenido de una actividad en formato Markdown. \\ \midrule
		ACC-11 & Exportar a \textit{JSON-LD} &  Exporta el grafo local a un archivo en formato \textit{JSON-LD}. \\ \midrule
...
Para poder dar cumplimiento a los requisitos de la aplicación, se definen en el cuadro \ref{tab:componentes} los componentes necesarios con sus ámbitos de responsabilidad.
	...
	\begin{table}
		...
		\begin{tabular}{p{2cm}p{3cm}p{10cm}}
			...
			COMP-14 & Visor de \textit{markdown} & Componente reutilizable para visualizar\textit{ markdown} renderizado en HTML. \\ 
