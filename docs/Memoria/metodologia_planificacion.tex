...
\noindent\begin{minipage}{\linewidth}
Prueba del acierto a la hora de elegir esta metodología es el cambio de necesidades y requisitos funcionales que tuvo lugar en la reunión presencial mantenida con el tutor, donde \textbf{la metodología aportó que no fuera necesario descartar ningún desarrollo o trabajo realizado hasta la fecha}. Permitió realizar una gestión del cambio efectiva, reorientando el trabajo a tiempo sin impactar en el diseño ni en la implementación existente. Finalmente, tanto la organización como la planificación de las tareas han sido lo suficientemente flexibles para conseguir un ritmo adecuado de desarrollo, reduciendo los puntos de bloqueo.
\end{minipage}
...
\begin{itemize}
	...
	\item El período de formación llevó al menos dos semanas más de lo esperado (en realidad, el aprendizaje del marco de trabajo \textit{Vue} ha estado presente a lo largo de prácticamente todo el desarrollo del proyecto).
...
\begin{itemize}
	...
	\item La curva de aprendizaje de \textit{Vue}, si bien es considerada más suave que la de sus competidores (\textit{React, Angular}) fue mayor de lo esperado. El poco o ningún grado de familiarización del autor del proyecto con las tecnologías de \textit{frontend} y, especialmente, \textit{Javascript ES6+}, no facilitó precisamente el aprendizaje.
	\item El\textbf{ bajo nivel de madurez de las bibliotecas existentes} para la manipulación de \gls{RDF} en \textit{Javascript}, así como la heterogeneidad y poca estabilidad de estas implementaciones, ha suscitado muchas dudas sobre cuáles utilizar y cómo enfocar su integración con marcos de trabajo más maduros como \textit{Vue}.
	\item La \textbf{práctica ausencia de bibliotecas} o componentes de interacción con \gls{SPARQL} \textbf{conformes a la especificación estándar de la interfaz} \textit{rdf.js}{\cite{rdfjs01}} (que, por otra parte, es un borrador de 2017) ha puesto en peligro la viabilidad del proyecto. La plataforma finalmente utilizada, \textit{Comunica}{\cite{comunica01}}, aún no tiene una versión estable en muchos de sus módulos (concretamente, el \textit{endpoint} \gls{SPARQL} utilizado para el proyecto se encuentra en esta situación), con lo que ha sido necesario estar en contacto directo con sus autores y colaborar con ellos en la revisión de defectos o \textit{bugs} y dependiendo, por tanto, de sus tiempos de respuesta (hay que tener en cuenta que la plataforma es \textit{opensource} y no existe acuerdo de nivel de servicio alguno.)
	\item La dificultad encontrada en llevar a cabo un análisis de requisitos a través de una plataforma de colaboración en línea como puede ser \textit{Skype}: para constatar este hecho no hay más que verificar el cambio de rumbo del proyecto una vez mantenida la reunión presencial, que sirvió para definir objetivos más claros y comprender mejor las necesidades del Departamento.
\end{itemize} 
...
Si bien para la planificación global del proyecto se han utilizado herramientas tales como el \textit{diagrama de Gantt}, para gestionar el trabajo del día a día ha sido necesario emplear otro enfoque. En el contexto de metodologías ágiles de desarrollo de software de tipo incremental e iterativo como \textit{Extreme Programming}, \textbf{las tareas de planificación adquieren un carácter adaptativo} muy distinto del que presenta una planificación tradicional con una metodología de desarrollo en cascada, por ejemplo.
...
Para gestionar estas tareas, se optó por utilizar como herramienta \textit{Kanban}\footnote{\url{https://es.atlassian.com/agile/kanban}}. Un tablero \textit{Kanban} puede definirse como un dispositivo de señalización que introduce el flujo de trabajo de un proceso a un ritmo manejable. Presenta las siguientes características\cite{kanban01}:
...
\begin{center}
	\begin{table}[htb]
		...
		\begin{tabular}{p{3cm}p{10cm}}
			...
			Columna & Descripción \\
			\toprule
			\textit{To-Do} & Tareas por realizar en orden de prioridad descendente \\ \midrule
			\textit{Work in progress} & Tareas realizándose en un momento dado. No más de dos o tres. \\ \midrule
			\textit{Stand-by} & Tareas a la espera por motivos ajenos al autor del proyecto. \\ \midrule
			\textit{Done} & Tareas finalizadas. \\ \midrule
			\textit{Discarded} & Tareas descartadas debido a cambios de diseño, requisitos, etc. \\ \midrule
			\textit{Doubts} & Dudas planteadas a lo largo del desarrollo del proyecto. \\ \midrule
			\textit{Resources} & Recursos documentales en la web útiles para del desarrollo del proyecto. \\
			\bottomrule
		\end{tabular}
		\caption{Diseño del tablero \textit{Kanban}.}
		\label{tabkanban}
	\end{table}
\end{center}
...
\begin{figure}[H]
	...
	\caption{Tablero \textit{Trello} del proyecto.}
\end{figure}
