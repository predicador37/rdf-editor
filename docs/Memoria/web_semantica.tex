...
Cabe comenzar el desarrollo de la presente memoria con una breve introducción a los fundamentos de la Web Semántica y las tecnologías que la sustentan: RDF y SPARQL. Asimismo, queda definido también el término de datos enlazados o \textit{linked data}, estrechamente relacionado con estos conceptos y de gran importancia hoy en día en la difusión de datos estructurados a través de Internet.
...
\begin{itemize}  
	\item  \textbf{RDF} (\textit{Resource Description Framework}), que permite expresar conocimiento en forma de tripletas o ternas (a modo de sujeto, verbo y objeto en una oración) utilizando URIs (\textit{Uniform Resource Indicators}\footnote{Cadenas de caracteres que identifican los recursos de una red de forma unívoca.}).
...
\begin{center}
	\begin{table}[htb]
		...
		\begin{tabular}{p{3cm}p{12cm}}
			...
			\textit{RDF Primer} & Un marco de trabajo para expresar información sobre recursos. \\ \midrule
			\textit{RDF Concepts \& Syntax} & Un marco de trabajo para representar información en la Web. \\ 
			\bottomrule
		\end{tabular}
...
Los nombres de estos recursos (así como de los predicados) han de ser globales y deberían identificarse unívocamente por un \textit{Uniform Resource Idenfifier} (URI).
...
Tal y como SQL provee (al menos, relativamente) un de un estándar de consulta  para las bases de datos relacionales, SPARQL ofrece un lenguaje de consulta estandarizado para grafos RDF. 

SPARQL es un acrónimo recursivo para \textit{SPARQL Protocol And RDF Query Language}; es a la vez un lenguaje de consulta de grafos RDF y un protocolo, dado que permite exponer servicios web que acepten consultas en dicho lenguaje.
...
\begin{itemize}  
	...
	\item Transformación de datos RDF desde un vocabulario a otro.
\end{itemize}
\end{minipage}

La estructura básica de una consulta SPARQL es la siguiente:

\begin{lstlisting}[basicstyle=\tiny, caption={Estructura básica de consulta SPARQL.},captionpos=b]
